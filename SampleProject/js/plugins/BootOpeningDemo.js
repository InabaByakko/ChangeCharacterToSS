...
BOPD.firstMapId  = Number(parameters['firstMapId'] || 0);
BOPD.firstStartX = Number(parameters['firstStartX'] || 0);
BOPD.firstStartY = Number(parameters['firstStartY'] || 0);
BOPD.openingDemoEnd = false;
BOPD.openingDemoSkipOk = false;
...
(function() {
    ...
    Game_Interpreter.prototype.pluginCommand = function (command, args)
    {
        ...
        if (!BOPD.openingDemoEnd)
        {
            if (command === 'BootOpeningDemo')
            {
                ...
                {
                    case 'end':
                        BOPD.openingDemoEnd = true;
                        // タイトル画面へ遷移、通常のScene_Bootを開始する
                        SceneManager.goto(Scene_Title);
                        Window_TitleCommand.initCommandPosition();
                        break;
                    case 'skipOK':
                        BOPD.openingDemoSkipOk = true;
                        break;
                    case 'skipNG':
                        BOPD.openingDemoSkipOk = false;
                        break;
    ...
    Scene_Boot.prototype.start = function()
    {
        ...
        else
        {
            ...
            // エラーチェック
            if (BOPD.firstMapId == 0) {
                throw new Error("(BootOpeningDemo) parameter firstMapId is invalid");
            }
            this.checkPlayerLocation();
            // マップ呼び出し
            $gamePlayer.reserveTransfer(BOPD.firstMapId,
                BOPD.firstStartX, BOPD.firstStartY);
